  ...
  try {
    ...
    if (typeof window !== "undefined" && process.env.NODE_ENV === "development") {
      ...
      if (!currentOrigin.includes("localhost:3000") && currentOrigin.includes("localhost")) {
        console.log(
          `[OneSignal] Skipping init - port mismatch. Current: ${currentOrigin}, Expected: ${expectedOrigin}`,
        );
        return false;
      }
    }
    ...
    if (errorMessage.includes("timeout")) {
      console.warn(
        "[OneSignal] Initialization timed out after 3s (non-critical - notifications disabled)",
      );
    } else if (
      process.env.NODE_ENV === "development" &&
      (errorMessage.includes("Can only be used on:") || errorMessage.includes("localhost"))
    ) {
...
export const getPermissionStatus = async (): Promise<"granted" | "denied" | "default"> => {
  ...
  try {
    ...
    const permission = await Promise.race([
      OneSignal.Notifications.permission,
      new Promise<boolean | "default">((resolve) => setTimeout(() => resolve("default"), 1000)),
    ]);
