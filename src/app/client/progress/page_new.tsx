...
import { ChartsAndGraphs } from '@/components/progress/ChartsAndGraphs'
import { 
  TrendingUp, 
  Dumbbell, 
  Trophy,
...
export default function ClientProgress() {
  ...
  const progressMenuItems = [
    ...
    },
    {
      ...
      description: 'Visual transformation tracker',
      icon: Camera,
    ...
    },
    {
      ...
      description: 'Detailed performance graphs',
      icon: BarChart3,
    ...
    },
    {
      ...
      description: 'Track weight, body fat, and more',
      icon: Scale,
          ...
          <div className="p-4 sm:p-6">
            <div className="max-w-7xl mx-auto space-y-6">
                  ...
                  photos={[
                    {
                      ...
                      tags: ['Front', 'Starting Point'],
                      isFavorite: false,
                      transformation: { weightChange: 0, bodyFatChange: 0, muscleGain: 0 }
                    }
                  ]}
                  loading={loading}
                  onUpload={() => console.log('Upload photo')}
                  onShare={(photo) => console.log('Share photo:', photo)}
                  onDelete={(photoId) => console.log('Delete photo:', photoId)}
                />
              )}
              ...
              {selectedView === 'charts' && (
                <ChartsAndGraphs 
                  loading={loading}
                  onExport={(chartId) => console.log('Export chart:', chartId)}
                  onShare={(chartId) => console.log('Share chart:', chartId)}
                  onRefresh={() => console.log('Refresh charts')}
                />
              )}
                  ...
                  <Card className={`border-0 ${theme.card} bg-white/95 dark:bg-slate-800/95 backdrop-blur-md overflow-hidden rounded-3xl`}>
                    <CardHeader className="p-6">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <CardTitle className={`text-2xl font-bold ${theme.text}`}>Body Measurements</CardTitle>
                          <p className={`${theme.textSecondary}`}>Track your physical changes</p>
                        </div>
                      </div>
                    </CardHeader>
                    <CardContent className="p-6">
                      <p className={`${theme.textSecondary} text-center py-8`}>Measurement tracking will be displayed here</p>
                    </CardContent>
                  </Card>
                </div>
              )}
                  ...
                  <Card className={`border-0 ${theme.card} bg-white/95 dark:bg-slate-800/95 backdrop-blur-md overflow-hidden rounded-3xl`}>
                    <CardHeader className="p-6">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <CardTitle className={`text-2xl font-bold ${theme.text}`}>Goals & Milestones</CardTitle>
                          <p className={`${theme.textSecondary}`}>Set and achieve your targets</p>
                        </div>
                      </div>
                    </CardHeader>
                    <CardContent className="p-6">
                      <p className={`${theme.textSecondary} text-center py-8`}>Goals tracking will be displayed here</p>
                    </CardContent>
                  </Card>
                </div>
              )}
