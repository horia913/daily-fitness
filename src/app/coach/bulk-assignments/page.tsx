...
export default function CoachBulkAssignmentsPage() {
  ...
  const [selectedAssignment, setSelectedAssignment] = useState<BulkAssignment | null>(null)
  ...
  const filteredAssignments = assignments.filter(assignment => {
    const matchesSearch = assignment.operation_name.toLowerCase().includes(searchTerm.toLowerCase())
    const matchesStatus = !filterStatus || assignment.status === filterStatus
    const matchesType = !filterType || assignment.operation_type === filterType
  ...
  const getAssignmentTypeIcon = (type: string) => {
    switch (type) {
      case 'program': return <Target className="w-4 h-4" />
      case 'workout': return <Dumbbell className="w-4 h-4" />
      case 'meal_plan': return <Apple className="w-4 h-4" />
  ...
  const getAssignmentTypeColor = (type: string) => {
    switch (type) {
      case 'program': return 'bg-gradient-to-r from-purple-500 to-purple-600'
      case 'workout': return 'bg-gradient-to-r from-blue-500 to-blue-600'
      case 'meal_plan': return 'bg-gradient-to-r from-green-500 to-green-600'
      default: return 'bg-gradient-to-r from-slate-500 to-slate-600'
    }
  }

  const getProgressPercentage = (assignment: BulkAssignment) => {
    return assignment.total_items > 0 ? (assignment.processed_items / assignment.total_items) * 100 : 0
  }

  const getSuccessRate = (assignment: BulkAssignment) => {
    return assignment.processed_items > 0 ? (assignment.success_items / assignment.processed_items) * 100 : 0
  }

  const loadStats = async () => {
    try {
      const { data, error } = await supabase.rpc(
        'get_bulk_assignment_stats',
        { p_coach_id: user?.id }
      )
  ...
  const handleRetryAssignment = async (assignment: BulkAssignment) => {
    if (!confirm(`Are you sure you want to retry the bulk assignment "${assignment.operation_name}"?`)) {
      return
    }
  ...
  const handleCancelAssignment = async (assignment: BulkAssignment) => {
    if (!confirm(`Are you sure you want to cancel the bulk assignment "${assignment.operation_name}"?`)) {
      return
    }
  ...
  const handleDeleteAssignment = async (assignment: BulkAssignment) => {
    if (!confirm(`Are you sure you want to delete the bulk assignment "${assignment.operation_name}"? This action cannot be undone.`)) {
      return
    }
          ...
          <div className="p-6">
            <div className="max-w-7xl mx-auto space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                {[...Array(4)].map((_, i) => (
                  <div key={i} className={`h-32 ${theme.card} ${theme.shadow} rounded-2xl`}>
              ...
              <div className="space-y-6">
                {[...Array(3)].map((_, i) => (
                  <div key={i} className={`h-48 ${theme.card} ${theme.shadow} rounded-2xl`}>
                    <div className="animate-pulse p-6">
                      <div className="h-6 bg-slate-200 dark:bg-slate-700 rounded-lg mb-3"></div>
                      <div className="h-4 bg-slate-200 dark:bg-slate-700 rounded-lg w-3/4 mb-2"></div>
          ...
          <div className="max-w-7xl mx-auto space-y-8">
            ...
            <div className="text-center space-y-4">
              ...
              <p className={`text-lg ${theme.textSecondary} max-w-2xl mx-auto`}>
                Efficiently assign workouts, meal plans, and programs to multiple clients
              </p>
            ...
            {stats && (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <Card className={`${theme.card} ${theme.shadow} rounded-2xl hover:scale-105 transition-all duration-300 group`}>
                  <CardContent className="p-6">
                    <div className="flex items-center gap-4">
                      <div className={`p-3 rounded-xl bg-gradient-to-r from-blue-500 to-blue-600 ${theme.shadow} group-hover:scale-110 transition-transform`}>
                        <BarChart3 className="w-6 h-6 text-white" />
                      </div>
                      <div>
                        <div className={`text-3xl font-bold ${theme.text} group-hover:text-blue-600 transition-colors`}>
                          {stats.totalOperations}
                        </div>
                        <div className={`text-sm ${theme.textSecondary} font-medium`}>Total Operations</div>
                      </div>
                    </div>
                  </CardContent>
                </Card>
                ...
                <Card className={`${theme.card} ${theme.shadow} rounded-2xl hover:scale-105 transition-all duration-300 group`}>
                  <CardContent className="p-6">
                    <div className="flex items-center gap-4">
                      <div className={`p-3 rounded-xl bg-gradient-to-r from-green-500 to-green-600 ${theme.shadow} group-hover:scale-110 transition-transform`}>
                        <CheckCircle className="w-6 h-6 text-white" />
                      </div>
                      <div>
                        <div className={`text-3xl font-bold ${theme.text} group-hover:text-green-600 transition-colors`}>
                          {stats.completedOperations}
                        </div>
                        <div className={`text-sm ${theme.textSecondary} font-medium`}>Completed</div>
                      </div>
                    </div>
                  </CardContent>
                </Card>
                ...
                <Card className={`${theme.card} ${theme.shadow} rounded-2xl hover:scale-105 transition-all duration-300 group`}>
                  <CardContent className="p-6">
                    <div className="flex items-center gap-4">
                      <div className={`p-3 rounded-xl bg-gradient-to-r from-purple-500 to-purple-600 ${theme.shadow} group-hover:scale-110 transition-transform`}>
                        <Users className="w-6 h-6 text-white" />
                      </div>
                      <div>
                        <div className={`text-3xl font-bold ${theme.text} group-hover:text-purple-600 transition-colors`}>
                          {stats.totalItemsAssigned}
                        </div>
                        <div className={`text-sm ${theme.textSecondary} font-medium`}>Items Assigned</div>
                      </div>
                    </div>
                  </CardContent>
                </Card>
                ...
                <Card className={`${theme.card} ${theme.shadow} rounded-2xl hover:scale-105 transition-all duration-300 group`}>
                  <CardContent className="p-6">
                    <div className="flex items-center gap-4">
                      <div className={`p-3 rounded-xl bg-gradient-to-r from-orange-500 to-orange-600 ${theme.shadow} group-hover:scale-110 transition-transform`}>
                        <TrendingUp className="w-6 h-6 text-white" />
                      </div>
                      <div>
                        <div className={`text-3xl font-bold ${theme.text} group-hover:text-orange-600 transition-colors`}>
                          {stats.successRate.toFixed(1)}%
                        </div>
                        <div className={`text-sm ${theme.textSecondary} font-medium`}>Success Rate</div>
                      </div>
                    </div>
                  </CardContent>
                </Card>
            ...
            <div className="space-y-6">
              {filteredAssignments.map(assignment => (
                <Card key={assignment.id} className={`${theme.card} ${theme.shadow} rounded-2xl hover:shadow-2xl transition-all duration-300 group`}>
                  <CardHeader className="pb-4">
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-2">
                          <div className={`p-2 rounded-lg ${getAssignmentTypeColor(assignment.operation_type)} ${theme.shadow}`}>
                        ...
                        <p className={`text-sm ${theme.textSecondary} mb-3`}>
                          {assignment.operation_type} assignment • {assignment.total_items} items
                        </p>
                        <div className="flex items-center gap-3 flex-wrap">
                          <Badge className={`${getStatusColor(assignment.status)} rounded-xl px-3 py-1`}>
                  ...
                  <CardContent className="pt-0">
                    ...
                    <div className="space-y-3 mb-6">
                      <div className="flex items-center justify-between">
                        ...
                        <span className={`text-sm font-bold ${theme.text}`}>
                          {assignment.processed_items} / {assignment.total_items} items
                        </span>
                      </div>
                      ...
                      <div className="text-center">
                        <span className={`text-sm font-medium ${theme.textSecondary}`}>
                          {getProgressPercentage(assignment).toFixed(1)}% Complete
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-3 gap-4 mb-6">
                      <div className="text-center p-4 bg-gradient-to-r from-green-50 to-green-100 dark:from-green-900/20 dark:to-green-800/20 rounded-xl border border-green-200 dark:border-green-700">
                        <div className="text-2xl font-bold text-green-600 dark:text-green-400">{assignment.success_items}</div>
                      ...
                      <div className="text-center p-4 bg-gradient-to-r from-red-50 to-red-100 dark:from-red-900/20 dark:to-red-800/20 rounded-xl border border-red-200 dark:border-red-700">
                        <div className="text-2xl font-bold text-red-600 dark:text-red-400">{assignment.failed_items}</div>
                    ...
                    {/* Enhanced Error Log */}
                    {assignment.error_log && assignment.error_log.length > 0 && (
                      <div className="p-4 bg-gradient-to-r from-red-50 to-red-100 dark:from-red-900/20 dark:to-red-800/20 border border-red-200 dark:border-red-700 rounded-xl">
                        <div className="flex items-center gap-2 mb-3">
                          ...
                          <div className="text-sm font-medium text-red-800 dark:text-red-400">Recent Errors:</div>
                        </div>
                        <div className="space-y-2">
                          {assignment.error_log.slice(0, 3).map((error, index) => (
                            <div key={index} className="text-sm text-red-700 dark:text-red-300 flex items-start gap-2">
                          ...
                          {assignment.error_log.length > 3 && (
                            <div className="text-sm text-red-600 dark:text-red-400 font-medium">
                              ... and {assignment.error_log.length - 3} more errors
                            </div>
                          )}
                        </div>
                      </div>
            ...
            {filteredAssignments.length === 0 && (
              <Card className={`${theme.card} ${theme.shadow} rounded-2xl text-center py-16`}>
                <CardContent>
                  <div className={`p-8 rounded-2xl ${theme.gradient} ${theme.shadow} w-32 h-32 mx-auto mb-8 flex items-center justify-center`}>
                    <Zap className="w-16 h-16 text-white" />
                  </div>
                  <h3 className={`text-2xl font-bold ${theme.text} mb-4`}>
                    {searchTerm || filterStatus || filterType ? 'No assignments found' : 'No bulk assignments yet'}
                  </h3>
