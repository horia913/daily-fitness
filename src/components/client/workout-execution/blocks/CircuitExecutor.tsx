...
export function CircuitExecutor({
  block,
  onBlockComplete,
  onNextBlock,
  e1rmMap = {},
  ...
  // Also add exercises from circuit_sets
  ...
  const handleComplete = () => {
    console.log("CircuitExecutor: handleComplete called", {
    ...
    try {
      ...
      console.log("CircuitExecutor: onBlockComplete called successfully");
    } catch (error) {
      console.error("CircuitExecutor: Error in handleComplete", error);
    }
  };
    ...
    <div className="space-y-4">
      ...
      {circuitSets.length > 0 ? (
        ...
        <div className="text-center py-8 text-slate-500 dark:text-slate-400">
          No exercises configured for this Circuit block.
        </div>
      )}
      ...
      {circuitSets.length > 0 && (
        <Button
          ...
          className="w-full bg-gradient-to-r from-violet-600 to-purple-600 hover:from-violet-700 hover:to-purple-700 text-white text-lg py-4"
        >
  ...
  const completeButton = (
    <Button
      onClick={(e) => {
        e.preventDefault();
        e.stopPropagation();
        console.log("CircuitExecutor: Complete Block button clicked");
        handleComplete();
      }}
  ...
  return (
    <>
      <BaseBlockExecutorLayout
        ...
        exerciseName={currentExercise?.exercise?.name || "Circuit"}
