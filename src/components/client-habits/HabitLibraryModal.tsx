...
function buildChipCounts(searchFiltered: HabitTemplateRow[]): Record<ChipFilterId, number> {
...
function sessionSummaryLine(ids: Set<string>, templates: HabitTemplateRow[]): string {
...
export type HabitLibraryModalProps = {
  ...
  setConfigureForm: React.Dispatch<React.SetStateAction<Record<string, string>>>
...
export function HabitLibraryModal({
  ...
  const chipCounts = useMemo(() => buildChipCounts(searchFiltered), [searchFiltered])
  ...
  const searchEmpty =
    debouncedSearch.trim().length > 0 && searchFiltered.length === 0 && pickableTemplates.length > 0
  ...
  const liveMsg =
    sessionIds.size > 0 ? `${sessionIds.size} habit(s) staged. ${footerSummary}` : ''

  if (!open || typeof document === 'undefined') return null

  return createPortal(
    <div className={styles.portalRoot} role="presentation">
      <div className={styles.backdrop} aria-hidden onClick={() => requestClose()} />
      <div
        ref={sheetRef}
        className={`${styles.sheet} ${styles.root}`}
        role="dialog"
        aria-modal="true"
        aria-labelledby={view === 'library' ? 'habit-lib-title' : 'habit-cfg-title'}
      >
        ...
        {view === 'library' ? (
          <>
            ...
            />
            <CategoryChips active={chip} onChange={setChip} counts={chipCounts} />
            <div className={styles.scrollBody}>
              {templatesLoading ? (
                <p style={{ color: 'var(--t3)', fontSize: 13, textAlign: 'center', padding: 24 }}>
                  Loading library…
                </p>
              ) : null}
              ...
              {!templatesLoading && searchEmpty ? (
                <EmptyHabitSearch query={debouncedSearch.trim()} onClear={() => setSearchDraft('')} />
              ) : null}
              {!templatesLoading &&
                !libraryFullyEmpty &&
                !searchEmpty &&
          ...
          <>
            <header className={styles.configureHead}>
              <div className={styles.titleRow}>
                ...
                <button type="button" onClick={requestClose} className={styles.modalCloseBtn} aria-label="Close">
            ...
            <div className={styles.configureBody}>
              <div className={styles.fieldStack}>
                {configureTemplate?.user_configurable_keys.map((key) => (
                  <div key={key}>
                    <label className={styles.fieldLabel} htmlFor={`habit-cfg-${key}`}>
                      {key.replace(/_/g, ' ')}
                    </label>
                    <input
                      id={`habit-cfg-${key}`}
