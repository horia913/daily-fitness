...
import GoalCard, {
  type GoalCardPillar,
} from "@/components/coach/client-detail/GoalCard";
...
function sourcePresentation(
  ...
  if (sourceType === "meal_plan") {
    const tracking =
      sourceConfig && typeof sourceConfig.tracking === "string"
        ? sourceConfig.tracking
        : null;
    if (tracking === "daily_macro") {
      const macro =
        sourceConfig && typeof sourceConfig.macro === "string"
          ? sourceConfig.macro
          : "";
      const label =
        macro === "protein_g"
    ...
    return {
      Icon: Apple,
      label: "Linked to meal plan adherence (sync pending)",
    };
  }
...
export default function ClientGoalsView({
  ...
  const latestSync = useMemo(() => {
    ...
    const latest = goals.reduce((acc, g) =>
      new Date(g.updated_at).getTime() > new Date(acc.updated_at).getTime()
        ? g
        : acc,
    );
                ...
                {items.map((goal) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {goal.notes ? (
                        <p className="text-xs mt-2 italic fc-text-dim">
                          {goal.notes}
                        </p>
                      ) : null}
                    </article>
