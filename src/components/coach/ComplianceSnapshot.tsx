...
export default function ComplianceSnapshot() {
  ...
  const loadComplianceData = async () => {
    ...
    try {
      ...
      const { data, error: mainError } = await supabase
        .rpc('get_client_compliance_scores', { coach_id_param: user.id })

      if (mainError) {
        // Silently try fallback function
        const { data: simpleData, error: simpleError } = await supabase
          .rpc('get_client_compliance_scores_simple', { coach_id_param: user.id })
        
        if (simpleError) {
          // Silently handle - function doesn't exist or has type mismatch
          setError('Compliance data unavailable - database functions need to be set up')
          setClients([])
        } else {
      ...
      setClients([
        {
          ...
          total_assigned: 5,
          total_completed: 3,
          last_workout_date: new Date(Date.now() - 2 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
          current_streak: 2,
        ...
        },
        {
          ...
          total_assigned: 6,
          total_completed: 5,
          last_workout_date: new Date(Date.now() - 1 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
          current_streak: 5,
        ...
        },
        {
          ...
          total_assigned: 7,
          total_completed: 3,
          last_workout_date: new Date(Date.now() - 3 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
          current_streak: 0,
  ...
  const calculateOverallCompliance = () => {
    ...
    const total = clients.reduce((sum, client) => sum + client.compliance_score, 0)
  ...
  const getComplianceCategories = (): ComplianceCategory[] => {
    ...
    const workoutCompliance = clients.reduce((sum, client) => sum + (client.workout_compliance || client.compliance_score), 0) / clients.length
    const nutritionCompliance = clients.reduce((sum, client) => sum + (client.nutrition_compliance || client.compliance_score), 0) / clients.length
    const habitCompliance = clients.reduce((sum, client) => sum + (client.habit_compliance || client.compliance_score), 0) / clients.length
    ...
    return [
      {
        ...
        target: 80,
        completed: clients.reduce((sum, client) => sum + client.total_completed, 0),
  ...
  const handleClientClick = (client: ClientCompliance) => {
    ...
    setSelectedClient({
      ...
      email: '', // Would fetch from profiles
      ...
      <CardHeader className="pb-4 relative z-10">
        <div className="flex items-center justify-between">
          <CardTitle className={`flex items-center gap-3 ${theme.text}`}>
            ...
            <div>
              <h2 className="text-lg sm:text-xl font-bold">Compliance Snapshot</h2>
              <p className={`text-sm ${theme.textSecondary}`}>Overall client adherence overview</p>
            </div>
          </CardTitle>
      ...
      <CardContent className="p-4 sm:p-6 relative z-10">
        {clients.length > 0 ? (
          <div className="space-y-6">
            ...
            <div className={`${theme.card} rounded-2xl p-6 border-2 border-blue-200 dark:border-blue-800 bg-gradient-to-br from-blue-50 to-green-50 dark:from-blue-900/20 dark:to-green-900/20`}>
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className={`text-lg font-bold ${theme.text}`}>Overall Compliance</h3>
                    <p className={`text-sm ${theme.textSecondary}`}>Average across all clients</p>
                  </div>
                </div>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between text-sm">
                  <span className={`${theme.textSecondary}`}>Progress to Excellence</span>
                  <span className={`font-medium ${getComplianceColor(overallCompliance)}`}>
                    {overallCompliance}% / 100%
                  </span>
                </div>
                <div className="w-full bg-slate-200 dark:bg-slate-700 rounded-full h-3">
                  <div 
                    className={`h-3 rounded-full ${getComplianceBarColor(overallCompliance)} transition-all duration-500`}
                    style={{ width: `${overallCompliance}%` }}
                  />
                </div>
                <p className={`text-sm font-medium ${getComplianceColor(overallCompliance)} mt-2`}>
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between mb-2">
                            ...
                            <div className="flex items-center gap-1">
                              ...
                              <Badge className={`text-xs ${getComplianceBadgeColor(category.compliance)}`}>
                                {category.compliance}%
                              </Badge>
                            </div>
                          </div>
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="w-full bg-slate-200 dark:bg-slate-700 rounded-full h-2">
                              <div 
                                className={`h-2 rounded-full ${getComplianceBarColor(category.compliance)} transition-all duration-500`}
                            ...
                            <div className="mt-3 pt-3 border-t border-slate-200 dark:border-slate-700">
                              <div className="space-y-2">
                                <div className="flex items-center justify-between text-xs">
                                  ...
                                  <span className={`font-medium ${theme.text}`}>{category.target}%</span>
                                </div>
                                <div className="flex items-center justify-between text-xs">
                                  <span className={`${theme.textSecondary}`}>Last Activity</span>
                                  <span className={`font-medium ${theme.text}`}>{category.lastActivity}</span>
                                </div>
                                {category.streak && (
                                  <div className="flex items-center justify-between text-xs">
                                    <span className={`${theme.textSecondary}`}>Best Streak</span>
                                    <span className={`font-medium ${theme.text}`}>{category.streak} days</span>
                                  </div>
                                )}
                                <Button
                                  variant="outline"
                      ...
                      >
                        <div className="flex items-center gap-4">
                          ...
                          <div className="relative">
                            ...
                            <div className={`absolute -bottom-1 -right-1 w-4 h-4 rounded-full border-2 border-white flex items-center justify-center ${getStatusIconColor(client.compliance_score)}`}>
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-2">
                              ...
                              <Badge className={`text-xs ${getComplianceBadgeColor(client.compliance_score)}`}>
                            ...
                            <div className="space-y-2">
                              <div className="flex items-center justify-between text-sm">
                                <span className={`${theme.textSecondary}`}>Compliance Score</span>
                                <span className={`font-semibold ${getComplianceColor(client.compliance_score)}`}>
                                  {client.compliance_score.toFixed(1)}%
                                </span>
                              </div>
                              <div className="w-full bg-slate-200 dark:bg-slate-700 rounded-full h-2">
                                <div 
                                  className={`h-2 rounded-full ${getComplianceBarColor(client.compliance_score)} transition-all duration-500`}
                            ...
                            <div className="flex items-center gap-4 mt-2 text-xs text-slate-500 dark:text-slate-400">
                              <div className="flex items-center gap-1">
                                ...
                                <span>{client.total_completed}/{client.total_assigned} workouts</span>
                              </div>
                              ...
                              <div className="flex items-center gap-1">
                                ...
                                <span>{formatLastActivity(client.last_workout_date)}</span>
                              </div>
                            </div>
                          </div>
                          ...
                          {/* Click indicator */}
                          <div className="flex items-center gap-2">
                            <span className={`text-xs ${theme.textSecondary}`}>View Details</span>
                            <ArrowRight className={`w-4 h-4 ${theme.textSecondary}`} />
                          </div>
                        </div>
                      </div>
                    )
            ...
            <div className="pt-4 border-t border-slate-200 dark:border-slate-700">
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-center">
                <div className={`${theme.card} rounded-xl p-4 border-2`}>
                  ...
                  <p className={`text-xs ${theme.textSecondary}`}>Total Clients</p>
                </div>
                <div className={`${theme.card} rounded-xl p-4 border-2 border-green-200 dark:border-green-800`}>
                  ...
                  <p className={`text-xs ${theme.textSecondary}`}>High Compliance</p>
                </div>
                <div className={`${theme.card} rounded-xl p-4 border-2 border-yellow-200 dark:border-yellow-800`}>
                  <p className={`text-2xl font-bold text-yellow-600 dark:text-yellow-400`}>
                    {clients.filter(c => c.compliance_score >= 60 && c.compliance_score < 80).length}
                  </p>
                  <p className={`text-xs ${theme.textSecondary}`}>Medium Compliance</p>
                </div>
                <div className={`${theme.card} rounded-xl p-4 border-2 border-red-200 dark:border-red-800`}>
                  ...
                  <p className={`text-xs ${theme.textSecondary}`}>Needs Attention</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="text-center py-12">
            ...
            <p className={`text-sm ${theme.textSecondary} mb-4`}>
              Client compliance data will appear here once clients start working out
            </p>
