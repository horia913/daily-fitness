...
export default function SimpleDebug() {
  ...
  const runTest = async () => {
    try {
      ...
      const { data: workouts, error: workoutError } = await supabase
        ...
        .select('id, template_id, scheduled_date, status')
      ...
      const results = [
        ...
        `Programs: ${programs?.length || 0} assignments (${programError ? 'Error: ' + programError.message : 'OK'})`,
  ...
  return (
    <div style={{
      ...
      <h3 style={{ margin: '0 0 10px 0', color: 'red' }}>🔍 SIMPLE DEBUG PANEL</h3>
      ...
      {testResult && (
        <p style={{ margin: '10px 0 0 0', color: testResult.includes('✅') ? 'green' : 'red' }}>
