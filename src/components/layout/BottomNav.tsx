...
import { Home, Dumbbell, Calendar, MessageCircle, User, Target, CreditCard, Utensils, TrendingUp, Award, Apple, Flag, Users, BarChart3, Grid, Sparkles, Activity, Zap, Leaf, CheckCircle, Trophy, Settings } from 'lucide-react'
      ...
      <div className="h-full px-2">
        <div className="flex items-center justify-between h-full max-w-md mx-auto">
          {navItems.map((item) => {
            ...
            const isHomeItem = item.href === '/client' || item.href === '/coach'
            const isWorkoutsItem = item.href === '/client/workouts' || item.href === '/coach/programs-workouts'
            const isNutritionItem = item.href === '/client/nutrition' || item.href === '/coach/nutrition'
            const isProgressItem = item.href === '/client/progress'
            const isProfileItem = item.href === '/client/profile'
            const isCoachWorkoutsItem = item.href === '/coach/programs-workouts'
            ...
            return (
              <Link
                ...
                } ${isHomeItem ? 'min-w-[60px]' : isWorkoutsItem || isNutritionItem || isProgressItem || isProfileItem ? 'min-w-[55px]' : 'min-w-[50px]'}`}
              >
                ...
                {/* Icon with special treatment for Home, Workouts, Nutrition, Progress, Profile, Coach Workouts, Coach Nutrition, Coach Messages, and Coach Menu */}
                <div className={`relative ${isHomeItem && isActive ? 'animate-pulse' : isWorkoutsItem && isActive ? 'animate-pulse' : isNutritionItem && isActive ? 'animate-pulse' : isProgressItem && isActive ? 'animate-pulse' : isProfileItem && isActive ? 'animate-pulse' : isCoachWorkoutsItem && isActive ? 'animate-pulse' : isCoachNutritionItem && isActive ? 'animate-pulse' : isCoachMessagesItem && isActive ? 'animate-pulse' : isCoachMenuItem && isActive ? 'animate-pulse' : ''}`}>
                  <Icon className={`${isHomeItem ? 'w-6 h-6' : isWorkoutsItem || isNutritionItem || isProgressItem || isProfileItem || isCoachWorkoutsItem || isCoachNutritionItem || isCoachMessagesItem || isCoachMenuItem ? 'w-5.5 h-5.5' : 'w-5 h-5'} transition-all duration-200 ${
                    isActive 
                      ? isWorkoutsItem 
                        ? 'text-orange-600' 
                        : isNutritionItem
                ...
                <span className={`text-xs font-semibold transition-all duration-200 ${
                  isActive 
                    ? isWorkoutsItem 
                      ...
                      : 'text-blue-600' 
                    : 'text-slate-500 group-hover:text-slate-700'
                } ${isHomeItem ? 'text-[11px]' : isWorkoutsItem || isNutritionItem || isProgressItem || isProfileItem || isCoachWorkoutsItem || isCoachNutritionItem || isCoachMessagesItem || isCoachMenuItem ? 'text-[10.5px]' : 'text-[10px]'}`}>
                  {item.label}
                </span>
