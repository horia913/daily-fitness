-- =====================================================
-- DailyFitness ClipCards Idempotent Setup
-- =====================================================
...
DO $$ 
BEGIN
    -- Add clipcard_types coach_id foreign key
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'clipcard_types_coach_id_fkey' 
        AND table_name = 'clipcard_types'
    ) THEN
    ...
    -- Add clipcards coach_id foreign key
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'clipcards_coach_id_fkey' 
        AND table_name = 'clipcards'
    ) THEN
        ALTER TABLE public.clipcards ADD CONSTRAINT clipcards_coach_id_fkey 
            FOREIGN KEY (coach_id) REFERENCES public.profiles(id) ON DELETE CASCADE;
    END IF;

    -- Add clipcards client_id foreign key
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'clipcards_client_id_fkey' 
        AND table_name = 'clipcards'
    ) THEN
    ...
    -- Add clipcards clipcard_type_id foreign key
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'clipcards_clipcard_type_id_fkey' 
        AND table_name = 'clipcards'
    ) THEN
...
-- =====================================================
-- Drop existing policies for clipcard_types
...
-- Drop existing policies for clipcards
...
-- Create new policies for clipcard_types
...
-- Create new policies for clipcards
