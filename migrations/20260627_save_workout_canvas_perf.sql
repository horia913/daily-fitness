-- ═══════════════════════════════════════════════════════════════════════════
-- save_workout_canvas performance fix (statement timeout 57014 on program save)
--
-- Run once in Supabase SQL Editor (new blank tab → paste → Run).
--
-- Symptoms: POST /rpc/save_workout_canvas → 500,
--   "[57014] canceling statement due to statement timeout"
--
-- Root cause: default statement_timeout (~8s on Supabase) plus slow DELETE on
--   workout_set_prescriptions without an index on slot_id (nested IN subquery).
-- Also clears legacy protocol child rows before set_entry delete.
-- ═══════════════════════════════════════════════════════════════════════════

-- 1) Indexes for template-scoped delete path
CREATE INDEX IF NOT EXISTS idx_wsp_slot_id
  ON public.workout_set_prescriptions (slot_id);

CREATE INDEX IF NOT EXISTS idx_workout_drop_sets_set_entry_id
  ON public.workout_drop_sets (set_entry_id);

CREATE INDEX IF NOT EXISTS idx_workout_cluster_sets_set_entry_id
  ON public.workout_cluster_sets (set_entry_id);

CREATE INDEX IF NOT EXISTS idx_workout_rest_pause_sets_set_entry_id
  ON public.workout_rest_pause_sets (set_entry_id);

CREATE INDEX IF NOT EXISTS idx_workout_time_protocols_set_entry_id
  ON public.workout_time_protocols (set_entry_id);

CREATE INDEX IF NOT EXISTS idx_workout_speed_sets_set_entry_id
  ON public.workout_speed_sets (set_entry_id);

CREATE INDEX IF NOT EXISTS idx_workout_endurance_sets_set_entry_id
  ON public.workout_endurance_sets (set_entry_id);

-- 2) Replace RPC (extends 20260614_save_workout_canvas_is_optional_coalesce.sql)
