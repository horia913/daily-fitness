-- ============================================================================
-- Extend get_client_dashboard RPC: wellness today, check-in streak, highlights,
-- athlete score. Dashboard page can use one RPC + API (no separate score/check-in).
-- ============================================================================
...
DECLARE
  ...
  -- Extended: wellness, check-in streak, highlights, athlete score
  ...
  v_first_day DATE;
  v_last_day DATE;
  -- Program progress (so API doesn't need buildProgramWeekState)
...
BEGIN
  ...
  -- Today's wellness log (for check-in card)
  ...
  -- Check-in streak: consecutive days with complete log (sleep_hours, sleep_quality, stress_level, soreness_level)
  ...
  -- Best leaderboard rank (min rank, optionally exercise name)
  SELECT jsonb_build_object(
    'rank', le.rank,
    'exerciseName', e.name
  ...
  -- Athlete score: latest row + history (last 12, oldest first for sparkline)
  ...
  -- Program progress: currentWeek, totalWeeks, completedCount, totalSlots, percent (for dashboard card)
...
'Client dashboard: profile, streak, weekly stats, body weight, todays workout, programProgress, wellness today, check-in streak, highlights, athlete score, scoreHistory. One RPC replaces 85+ queries.';