-- ============================================================================
-- Migration: Patch get_client_dashboard RPC for canonical column names
-- Date: 2026-02-09
-- Purpose:
--   The canonical program_progress table now uses:
--     current_week_number (1-based) instead of current_week_index (0-based)
--     current_day_number  (1-based) instead of current_day_index  (0-based)
--   This migration updates the RPC to read the new column names and
--   subtract 1 where 0-based indices are needed for CTE lookups.
-- ============================================================================
...
BEGIN
  ...
  DECLARE
    ...
    v_current_week_number INT;  -- 1-based (canonical)
  BEGIN
    -- Read current_week_number (1-based) from canonical program_progress
    SELECT pa.program_id, pp.current_week_number
    INTO v_active_program_id, v_current_week_number
    ...
    IF v_active_program_id IS NOT NULL THEN
      -- Convert 1-based week number to 0-based index for CTE lookup
      ...
      current_week AS (
        ...
        WHERE week_index = COALESCE(v_current_week_number - 1, 0)
      )
  ...
  BEGIN
    ...
    IF v_program_assignment.id IS NOT NULL THEN
      -- Get progress using canonical column names (1-based)
      ...
      IF v_program_progress IS NOT NULL AND NOT COALESCE(v_program_progress.is_completed, false) THEN
        ...
        -- Convert 1-based current_week_number to 0-based index for CTE lookup
        WITH week_numbers AS (
          SELECT DISTINCT week_number
          FROM program_schedule
        ...
        current_week AS (
          SELECT week_number FROM indexed_weeks
          WHERE week_index = COALESCE(v_program_progress.current_week_number - 1, 0)
        ),
        days_in_week AS (
...
Updated 2026-02-09: reads current_week_number/current_day_number (1-based) from canonical program_progress.';


-- ============================================================================
-- PART 2: Patch get_client_workout_summary RPC
-- ============================================================================
...
DECLARE
  ...
  v_has_schedule_row BOOLEAN := FALSE;
BEGIN
  v_client_id := auth.uid();
  IF v_client_id IS NULL THEN
    RAISE EXCEPTION 'Not authenticated';
  ...
  SELECT
    ...
    pa.start_date, pa.duration_weeks, wp.name AS program_name,
    wp.description AS program_description, wp.difficulty_level,
  ...
  IF v_active_program_assignment.id IS NOT NULL THEN
    -- Read canonical 1-based columns
    SELECT pp.program_assignment_id AS id, pp.current_week_number, pp.current_day_number, pp.is_completed
    INTO v_program_progress
    FROM program_progress pp
    ...
    IF v_program_progress.id IS NOT NULL AND NOT v_program_progress.is_completed THEN
      ...
      SELECT
        diw.id AS schedule_id, diw.template_id, diw.week_number, diw.day_of_week, diw.day_index,
        wt.name AS template_name, wt.description AS template_description, wt.estimated_duration
      INTO v_current_schedule_row
      FROM days_in_week diw
      LEFT JOIN workout_templates wt ON wt.id = diw.template_id
      WHERE diw.day_index = COALESCE(v_program_progress.current_day_number - 1, 0);
      v_has_schedule_row := FOUND;
    END IF;
  END IF;

  IF v_has_schedule_row AND v_current_schedule_row.schedule_id IS NOT NULL THEN
    v_todays_workout := jsonb_build_object(
      'hasWorkout', true,
      ...
      'currentWeekIndex', COALESCE(v_program_progress.current_week_number - 1, 0),
      'currentDayIndex', COALESCE(v_program_progress.current_day_number - 1, 0)
    );
  ELSE
    SELECT jsonb_build_object(
      'hasWorkout', true,
  ...
  IF v_active_program_assignment.id IS NOT NULL THEN
    ...
    BEGIN
      ...
      v_current_program := jsonb_build_object(
        ...
        'current_week', COALESCE(v_program_progress.current_week_number, 1),
  ...
  INTO v_weekly_progress, v_weekly_stats, v_all_time_volume
  ...
  BEGIN
    ...
    FROM workout_logs wl
    WHERE wl.client_id = v_client_id AND wl.completed_at IS NOT NULL;
    IF v_active_program_assignment.id IS NOT NULL AND v_program_progress.current_week_number IS NOT NULL THEN
      WITH week_numbers AS (
  ...
  SELECT COALESCE(SUM(total_weight_lifted), 0)::NUMERIC INTO v_all_time_volume
  FROM workout_logs
  ...
  FROM (
    SELECT jsonb_build_object(
      'hasWorkout', true, 'templateId', wa.workout_template_id, 'templateName', COALESCE(wa.name, 'Completed Workout'),
      'templateDescription', '', 'weekNumber', 0, 'programDay', 0, 'estimatedDuration', wl.total_duration_minutes,
      'difficultyLevel', 'intermediate', 'exercises', '[]'::jsonb, 'generatedAt', wl.completed_at,
      'message', 'Workout completed', 'completed', true, 'completedAt', wl.completed_at
    ) AS history_row, wl.completed_at
    FROM workout_logs wl
  ...
  -- 8. All assigned workouts
  WITH workout_assignments_enriched AS (
    SELECT wa.id, wa.workout_template_id, wa.scheduled_date, wa.assigned_date, wa.status, wa.name, wa.description, wa.coach_id, wa.created_at, 'workout' AS type,
    ...
    FROM workout_assignments wa LEFT JOIN profiles p ON p.id = wa.coach_id
    WHERE wa.client_id = v_client_id AND wa.status IN ('assigned', 'active', 'in_progress')
  ),
  program_assignments_enriched AS (
    SELECT pa.id, pa.program_id AS workout_template_id, pa.start_date AS scheduled_date, pa.start_date AS assigned_date, pa.status, COALESCE(pa.name, wp.name) AS name, COALESCE(pa.description, wp.description) AS description, pa.coach_id, pa.created_at, 'program' AS type,
      jsonb_build_object('id', pa.program_id, 'name', COALESCE(pa.name, wp.name, 'Program'), 'description', COALESCE(pa.description, wp.description), 'duration_weeks', COALESCE(pa.duration_weeks, wp.duration_weeks)) AS workout_templates,
      jsonb_build_object('id', p.id, 'first_name', p.first_name, 'last_name', p.last_name, 'avatar_url', p.avatar_url) AS profiles
    FROM program_assignments pa LEFT JOIN workout_programs wp ON wp.id = pa.program_id LEFT JOIN profiles p ON p.id = pa.coach_id
    WHERE pa.client_id = v_client_id
  )
  SELECT COALESCE(jsonb_agg(jsonb_build_object('id', id, 'workout_template_id', workout_template_id, 'scheduled_date', scheduled_date, 'assigned_date', assigned_date, 'status', status, 'name', name, 'description', description, 'coach_id', coach_id, 'created_at', created_at, 'type', type, 'workout_templates', workout_templates, 'profiles', profiles) ORDER BY created_at DESC), '[]'::jsonb)
  ...
  -- 9. This week assignments
  SELECT COALESCE(jsonb_agg(jsonb_build_object('id', wa.id, 'workout_template_id', wa.workout_template_id, 'scheduled_date', wa.scheduled_date, 'status', wa.status, 'name', wa.name, 'completed', wl.id IS NOT NULL, 'completed_at', wl.completed_at, 'duration_minutes', wl.total_duration_minutes)), '[]'::jsonb)
  INTO v_this_week_assignments
  FROM workout_assignments wa
  LEFT JOIN workout_logs wl ON wl.workout_assignment_id = wa.id AND wl.completed_at IS NOT NULL AND wl.completed_at >= v_monday AND wl.completed_at <= v_sunday + INTERVAL '1 day'
  WHERE wa.client_id = v_client_id AND wa.scheduled_date >= v_monday AND wa.scheduled_date <= v_sunday;

  -- 10. Completed programs
  SELECT COALESCE(jsonb_agg(jsonb_build_object('id', pa.id, 'program_id', pa.program_id, 'name', COALESCE(pa.name, wp.name), 'completed_at', pa.updated_at)), '[]'::jsonb)
  INTO v_completed_programs
  FROM program_assignments pa LEFT JOIN workout_programs wp ON wp.id = pa.program_id
  ...
  IF v_has_schedule_row AND v_current_schedule_row.schedule_id IS NOT NULL THEN
    v_schedule_id_by_template := jsonb_build_object(v_current_schedule_row.template_id::TEXT, v_current_schedule_row.schedule_id);
  ELSE
    v_schedule_id_by_template := '{}'::jsonb;
  END IF;
...
-- ============================================================================
-- PART 3: Patch get_coach_pickup_workout RPC
-- ============================================================================
...
BEGIN
  ...
  SELECT id, role, first_name, last_name INTO v_coach_profile
  FROM profiles WHERE id = v_coach_id;
  ...
  SELECT client_id, status INTO v_client_relation
  FROM clients WHERE coach_id = v_coach_id AND client_id = p_client_id;
  
  IF v_client_relation.client_id IS NULL THEN
    RAISE EXCEPTION 'Client not found or does not belong to this coach';
  END IF;
  
  SELECT id, first_name, last_name, avatar_url INTO v_client_profile
  FROM profiles WHERE id = p_client_id;
  
  SELECT pa.id, pa.program_id, pa.client_id, pa.coach_id, pa.name, pa.status,
         pa.duration_weeks, pa.total_days, pa.created_at
  ...
  -- Read canonical 1-based columns
  SELECT program_assignment_id AS id, current_week_number, current_day_number, is_completed
  INTO v_program_progress
  ...
  IF v_program_progress.id IS NULL THEN
    ...
    RETURNING program_assignment_id AS id, current_week_number, current_day_number, is_completed
    INTO v_program_progress;
  END IF;
  ...
  IF v_program_progress.is_completed THEN
    RETURN jsonb_build_object(
      ...
      'current_day_index', COALESCE(v_program_progress.current_day_number - 1, 0),
      'is_completed', true
    );
  END IF;
  
  -- Convert 1-based to 0-based for CTE index lookup
  WITH week_numbers AS (
    SELECT DISTINCT week_number
    FROM program_schedule
  ...
  SELECT diw.schedule_id, diw.program_id, diw.week_number, diw.day_of_week,
         diw.template_id, diw.day_index,
  ...
  IF v_current_schedule_row.schedule_id IS NULL THEN
    ...
    BEGIN
      ...
      FROM program_schedule WHERE program_id = v_active_assignment.program_id;
      IF v_schedule_count = 0 THEN
        RETURN jsonb_build_object(
          'error', 'Program schedule not configured',
          'message', 'No training days found in program_schedule for this program.',
          'program_id', v_active_assignment.program_id
        );
      ELSE
        RETURN jsonb_build_object(
          'error', 'Invalid progress state',
          'message', 'week_number=' || v_program_progress.current_week_number || ', day_number=' || v_program_progress.current_day_number || ' is out of bounds',
  ...
  SELECT id, name, description, estimated_duration INTO v_template
  FROM workout_templates WHERE id = v_current_schedule_row.template_id;
  
  SELECT COALESCE(jsonb_agg(
    jsonb_build_object(
      'id', wb.id, 'block_type', wb.block_type, 'block_name', wb.block_name, 'block_order', wb.block_order,
      'exercises', (
        SELECT COALESCE(jsonb_agg(
          jsonb_build_object('id', wbe.id, 'exercise_id', wbe.exercise_id, 'exercise_name', e.name,
            ...
            'weight_kg', wbe.weight_kg, 'rest_seconds', wbe.rest_seconds)
  ...
  INTO v_blocks
  FROM workout_blocks wb WHERE wb.template_id = v_current_schedule_row.template_id;
