-- ============================================================================
-- Extend get_client_nutrition_page RPC: add weeklyCompliance and allFoods
-- Removes need for separate goals (nutrition pillar), meal_completions (compliance), and foods queries on Fuel page.
-- ============================================================================
...
BEGIN
  ...
  IF v_assignment IS NULL THEN
    RETURN jsonb_build_object(
      ...
      'allFoods', (SELECT COALESCE(jsonb_agg(row_to_json(f)), '[]'::jsonb) FROM (
...
'Returns all data for the client Fuel/nutrition page: active assignment, daily selection, meals with food items/options/completions, nutrition goals, weekly compliance (meals_completed per day), and all foods. One RPC replaces 20+ queries.';
