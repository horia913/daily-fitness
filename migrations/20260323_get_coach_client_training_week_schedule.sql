-- Add weekSchedule (all program_schedule rows for display week + template names + completion flags).
-- Fixes Training tab strip when all slots are optional or weekDays-only logic missed rows.
