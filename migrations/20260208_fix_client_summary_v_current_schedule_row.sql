-- ============================================================================
-- Fix: get_client_workout_summary - avoid "record v_current_schedule_row is not assigned"
-- When client has no active program or no program progress, the SELECT INTO
-- never runs, so v_current_schedule_row is unassigned. Only reference it when
-- v_has_schedule_row is TRUE (set from FOUND after the SELECT INTO).
-- ============================================================================
...
BEGIN
  ...
  -- 6. Get weekly progress and stats (raw values; inner block overwrites v_weekly_progress/v_weekly_stats with jsonb)
  SELECT 
  ...
  BEGIN
    ...
    IF v_active_program_assignment.id IS NOT NULL AND v_program_progress.current_week_index IS NOT NULL THEN
      ...
      current_week AS (
        SELECT week_number FROM indexed_weeks WHERE week_index = v_program_progress.current_week_index
      )
  ...
  -- 11. Lookup maps (only use v_current_schedule_row when assigned)
...
'Returns comprehensive workout summary for the authenticated client. Uses auth.uid() internally. Fix: v_has_schedule_row guards use of v_current_schedule_row.';
