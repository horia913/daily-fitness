-- =============================================================================
-- Step 12 — assign_program_instance: copy phase_label; stop copying goal/profile
-- ONE-PASTE for Supabase SQL editor. Columns goal/custom_goal_label/
-- progression_profile remain on tables (gated drop later); omitted here so DB
-- defaults apply (goal='custom', progression_profile='none').
-- Prerequisite: training_blocks.phase_label + program_instance_phases.phase_label
-- =============================================================================
...
COMMENT ON FUNCTION public.assign_program_instance(uuid, uuid, uuid, date, text, text, text) IS
  'Deep-copies master program to client instance. Phases copy phase_label (not goal/profile); retired columns get DB defaults.';
