-- Pre-Wave2B backup: live public function definitions
-- Captured: 2026-07-11
-- Source: Supabase MCP execute_sql (pg_get_functiondef), read-only
-- Worklist: Wave 2B guard-definer-functions.sql (22 pronames, incl. get_coach_dashboard overloads)
-- Restore: paste individual CREATE OR REPLACE blocks if rollback needed

begin;

-- add_session_feedback(uuid,uuid,integer,text)
...
-- cancel_session(uuid,uuid)
CREATE OR REPLACE FUNCTION public.cancel_session(p_session_id uuid, p_user_id uuid)
 RETURNS void
 LANGUAGE plpgsql
 SET search_path TO 'public', 'extensions'
AS $function$
...
BEGIN
    ...
    -- Check if cancellation is at least 8 hours in advance
    IF session_record.time_slot_id IS NOT NULL THEN
        DECLARE
            time_slot RECORD;
        BEGIN
            SELECT * INTO time_slot FROM public.coach_time_slots
            WHERE id = session_record.time_slot_id;
            
            IF time_slot.date = CURRENT_DATE AND time_slot.start_time <= (CURRENT_TIME + INTERVAL '8 hours') THEN
                RAISE EXCEPTION 'Sessions can only be cancelled at least 8 hours in advance';
            END IF;
        END;
    END IF;
    ...
    -- Refund time slot if session was booked
    IF session_record.time_slot_id IS NOT NULL THEN
    ...
    -- Refund clipcard session if applicable
    IF session_record.clipcard_id IS NOT NULL THEN
        UPDATE public.clipcards 
        SET sessions_used = sessions_used - 1
        WHERE id = session_record.clipcard_id;
    END IF;
END;
$function$
...
-- complete_workout(uuid,integer,integer,uuid,integer,text)
...
-- extend_clipcard_validity(uuid,uuid,integer)
...
-- generate_daily_workout(uuid,uuid,date)
