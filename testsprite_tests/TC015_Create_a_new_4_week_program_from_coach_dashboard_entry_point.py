...
async def run_test():
    ...
    try:
        ...
        # -> Type the coach email into the email field, type the password into the password field, then click the Sign In button.
        ...
        # -> Click 'Training' in the bottom navigation to open the Training screen (use element index 408).
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/nav/div/a[3]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Click the 'Manage Programs' / Manage Programs button (element index 724) to open program management and reveal the 'Create New' or '+' program button.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div[2]/main/div/div[2]/div/div/div/div/a/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Click the 'Create Program' button (element index 828) to open the program creation screen and then verify the 'Basic Info' text is visible.
        frame = context.pages[-1]
        # Click element
        ...
        await expect(frame.locator('text=Basic Info').first).to_be_visible(timeout=3000)
