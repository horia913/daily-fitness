...
async def run_test():
    ...
    try:
        ...
        # -> Input email and password into the login form and click the Sign In button.
        ...
        # -> Click 'Training' in the bottom navigation to open the Training section and proceed to create a program.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/nav/div/a[3]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Click the 'Training' link in the bottom navigation to open the Training section (element index 409).
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/nav/div/a[4]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Click 'Training' in the bottom navigation to open the Training section (use element index 408).
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/nav/div/a[3]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        ...
        await expect(frame.locator('text=Week 1').first).to_be_visible(timeout=3000)
